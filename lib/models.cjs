'use strict';
/**
 * models.cjs — the injectable model layer shared by every QUORUM lane.
 *
 * QUORUM never calls a specific vendor. Instead it resolves three roles through an adapter:
 ...
 *
 * Set env QUORUM_MODELS to a module path exporting those three functions to use real models
...
function resolveModels() {
  ...
  const p = process.env.QUORUM_MODELS;
  if (p) {
    try {
