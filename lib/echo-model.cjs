 ...
 *
 * It performs NO network calls and needs NO API key. Its only job is to make every Fusion lane
 ...
 * the judge speaks the two prompt shapes Fusion uses — the batched @@ITEM@@ sentinel format used
 * by spec-fusion, and the single-candidate format used by full-fusion.
 *
 * Replace this with a real adapter (see examples/models-echo.cjs) by setting FUSION_MODELS.
 */
...
function codeStub(prompt) {
  ...
  return '```js\n' +
    ...
    `function ${name}(input) {\n  // TODO: replace the built-in echo model with a real one via FUSION_MODELS\n  return input;\n}\n` +
    `module.exports = ${name};\n` +
    '```';
}
