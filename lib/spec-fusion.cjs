'use strict';
/**
 * spec-fusion.cjs — QUORUM's batch lane (Lane 2).
 *
...
const DATA = process.env.QUORUM_DATA_DIR ? path.resolve(process.env.QUORUM_DATA_DIR) : path.join(ROOT, 'data');
...
function deterministicCheck(kind, text) {
  ...
  const tmp = path.join(os.tmpdir(), `quorum-${process.pid}-${crypto.randomBytes(4).toString('hex')}.cjs`);
