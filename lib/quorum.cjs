'use strict';
/**
 * quorum.cjs — the front door. optimize() routes each task to the cheapest lane that still verifies.
 *
 ...
 *
 *   const { optimize } = require('./quorum.cjs');
...
function delegateEligible(input, opts = {}, kind) {
  if (process.env.QUORUM_DELEGATE === '0') return false;             // kill-switch
  ...
  if (CODE_KINDS.has(k)) {
    if (process.env.QUORUM_DELEGATE_CODE === '0') return false;
...
// CLI: node lib/quorum.cjs [--kind=code] [--full] [--batch] [--task-file=f] "task" ["task2" ...]
if (require.main === module) {
  (async () => {
    ...
    if (!inputs.length) { console.log('Usage: node lib/quorum.cjs [--kind=code] [--full] [--batch] [--task-file=f] "task" ...'); process.exit(0); }
