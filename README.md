<!-- QUORUM — white-label. No personal or company identifiers in this file by design. -->

<p align="center">
  <img src="assets/hero.svg" alt="QUORUM" width="100%">
</p>
...
node lib/quorum.cjs "summarize the CAP theorem in two sentences"

# 2. force a lane / set a kind
node lib/quorum.cjs --kind=code --full "write a debounce function with a cancel method"

# 3. batch (spec-fusion lane) — one task per line
node lib/quorum.cjs --batch --task-file=tasks.txt --kind=code

# 4. see the router pick lanes across a mixed set
node examples/demo.cjs

# 5. plug in your own models (author / crossWatch / judge)
QUORUM_MODELS=./examples/models-echo.cjs node lib/quorum.cjs --full "design a rate limiter"
```

> Out of the box QUORUM uses a deterministic built-in echo model, so every lane runs with no API key — ideal for tests and CI. Point QUORUM_MODELS at an adapter exporting { author, crossWatch, judge } to use real models. The spec-fusion draft cache persists to ./data (gitignored). Every lane is fail-open and never throws.
...
quorum/
├── lib/
│   ├── quorum.cjs          ← the front door: optimize() router + delegateEligible + full-fusion lane
│   ├── spec-fusion.cjs     ← the batch lane: draft → batched judge → residual re-work → cache
│   ├── models.cjs          ← resolves your QUORUM_MODELS adapter, or the built-in echo model
...
<p align="center"><sub>QUORUM · delegate · spec-fusion · full-fusion · MIT</sub></p>
